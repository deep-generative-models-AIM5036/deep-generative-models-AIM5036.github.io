...
![Demo](/Glow_img//DDPM_img/Untitled%2039.png)
...
⑨: **Markov chain의 성질로 나타낼 수 있는 부분입니다.** **사실상 loss term을 만들어낼 때 제일 핵심이 되는 구간이라고 생각합니다.** (VAE에서의 ${q(x_T|x_0) \over q(x_T|x_0)}$ 곱하는 것에 버금가는 수식 전개 발상) 이 과정을 통해서 수식 내의 *p*와 *q* distribution이 같은 condition으로부터 같은 target distribution을 나타내게 되었습니다.
