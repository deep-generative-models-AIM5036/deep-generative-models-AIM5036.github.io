...
위에서 볼 수 있듯이 이제는 $z^{i}$를 $q(z)$에서 샘플링하면서 원래의 train objective값을 구할 수 있다. 따라서 원래의 train objective가 $\sum_{i}^{N}log \sum_{z}^{K}p_{z}(z)p_{\theta}(x^{(i)} \mid z)$이었다면, 다음과 같이 변경된다. 
...
여기서 importance weight는 다음과 같고, $w(x,h,\theta) = \frac{p(x,h \mid \theta)}{q(h \mid x,\theta)}$ normalized importance weight는 다음과 같다. $\tilde{w_i}= w_i/\sum_{i=1}^{k}w_i$
...
마지막으로 importance weight를 생각해보면 다음과 같다  $w(x,z) = \frac{p(x,z)}{q(z \mid x)}$
