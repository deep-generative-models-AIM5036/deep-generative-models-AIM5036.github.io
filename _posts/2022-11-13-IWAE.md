...
<p align="center"><img src="/assets/IWAE_img/Figure3.jpg" width="450" height="400"></p>

<div align="center">
(그림 3 K-Clusters) <sup>1</sup>
</div> 

이해를 돕기 위해 likelihood evaluation을 뜻하는 3번식 $p_{\theta}(x) = \sum_{z} P_{z}(z)p_{\theta}(x \mid z)$을 다시 한 번 본다. 
만일 우리가 MNIST데이터를 다루고 있고 (K=10), 숫자 1에 해당하는 데이터  $x^{(i)}$를 추출했다면, 
$p_{\theta}(x) = \frac{1}{10}\times p_{\theta}(x^{(i)} \mid z_{1}) + \frac{1}{10}\times0 + ...  + \frac{1}{10}\times0$
가 될 것이다. $p_{\theta}(x^{(i)} \mid z_{2})$를 포함한 다른 cluster에서 숫자 1에 해당하는 데이터가 뽑힐 확률은 0이기 때문이다.  
...
다음의 그림을 통해 어떠한 경우에 문제가 되는 지 다시 한 번 보겠다.

<img src="/assets/IWAE_img/Figure5.jpg" width="450" height="400">

위의 그림에서 붉은색 $f(z)$가 $p_{\theta}(x^{(i)} \mid z_{k}^{i})$를 나타낸다. 

그림에서 처럼 $p_{\theta}(x^{(i)} \mid z_{k}^{i})$가 높은 곳에서 $p_{z}(z)$가 낮으면 문제가 된다. $p_{z}(z)$에 따라 샘플된 데이터가 informative하지 않기 때문이다. 

## check

$q(z \mid x)$가 논문을 포함한 VAE모델에서 recognition network, $p(z \mid x)$를 poster라고 지칭한다는 것을 알고 있다면, “VAE harshly penalizes approximate posterior samples which are unlikely to explain data, even if the recognition network puts much of its probability mass on good explanations.”라는 문장을 여기서 이해할 수 있다. VAE는 approximate posterior, $q(z)$에서 뽑은 sample에서 $p_{\theta}(x^{(i)} \mid z_{k}^{i})$값이 높더라도 $p_{z}(z)$값이 낮다면 해당 샘플을 사용하지 못한다. 앞서 보았듯이 likelihood evaluation이 0에 가까워지기 되기 때문이다.    
## check

이러한 경우에 Importance Sampling기법을 사용해서 $p_{\theta}(x^{(i)} \mid z_{k}^{i})$에 대해 informative한 q distribution을 새롭게 정의하고, q distribution을 따라 샘플링한 데이터로 $p_{z}(z)$에 대한 기댓값을 찾는다. 
Importance sampling의 수식 유도 과정은 다음과 같다. 편의를 위해 discrete한 데이터 분포를 가정하고, $p_{\theta}(x^{(i)} \mid z_{k}^{i})$를 $f(z)$로 놓는다.
...
위에서 볼 수 있듯이 이제는 $z^{i}$를 $q(z)$에서 샘플링하면서 원래의 train objective값을 구할 수 있다. 따라서 원래의 train objective가 $$\sum_{i}^{N}log \sum_{z}^{K}p_{z}(z)p_{\theta}(x^{(i)} \mid z)$$이었다면, 다음과 같이 변경된다. $$\approx \sum_{i}log\frac{1}{K}\sum_{k=1}^{K}\frac{p_{z}(z_{k}^{(i)})}{q(z_{k}^{(i)})}p_{\theta}(x^{(i)} \mid z_{k}^{(i)}) \quad with \quad z_{k}^{(i)} \sim q(z_{k}^{(i)})$$로 바뀌게 된다. 
...
$$w(x,h,\theta) = \frac{p(x,h \mid \theta)}{q(h \mid x,\theta)}$$ 
...
$$\nabla_{\theta}log w(x,h(\epsilon_i,x,\theta),theta) = \nabla_{\theta}log p(x,h(x,\epsilon_i,\theta) \mid \theta) - \nabla_{\theta}log q(h(x,\epsilon_i,\theta) \mid x,\theta)$$
...
$$L_k = E_{h}[log\frac{1}{k}\sum_{i=1}^k\frac{p(x,h_i)}{q(h_i \mid x)}] \geq E_{h}[E_{I={i_1,...,i_m}}[log\frac{1}{m}\sum_{j=1}^m \frac{p(x,h_{ij})}{q(h_{ij} \mid x)}]] = L_m$$
...
<p align="center"><img src="/assets/IWAE_img/Figure6.jpg" width="600" height="400"></p>
<div align="center">
(그림 6 Table 1)
</div> 
...
<p align="center"><img src="/assets/IWAE_img/Figure7.jpg" width="500" height="450"></p>
<div align="center">
(그림 7 Correlation between active units and KL) <sup>5</sup>
</div> 


Inactive units은 collapsed to the prior문제를 정량적으로 나타낸다. 다른 표현으로는 over-pruning problem이라고 부른다. VAE Objective function에서 KL term을 optimize하는 게 더 쉽기 때문에 빠르게 해당 term을 0으로 만들게 되면 active units, 혹은 stochastic factor라고 불리는 components들이 죽어버리게 되고, 한 번 죽어버린 units들은 다시 살아날 수 없기 때문에 다양하고 meaningful한 representation을 배울 수 없는 현상을 의미한다. 즉, 모든 데이터를 표현하기 위해서는 D-dimension이 필요할지라도, each individual example은 훨씬 작은 d-dimension으로도 표현 할 수 있다. D-dimension과 더 작은 d-dimension간의 차이만큼 inactive units가 발생한다고 볼 수 있다. 이러한 내용을 그림 7을 통해 볼 수 있다. 그림 7의 출처 논문 title은 “Unit activity and KL term for a 50-unit VAE (sorted by activity)“이다. 따라서 activity와 KL term간의 correlation을 보여주기 위한 그림이다.
...
<p align="center"><img src="/assets/IWAE_img/Figure8.jpg" width="600" height="300"></p>
<div align="center">
(그림 8 테이블 2)
</div> 
...
<p align="center"><img src="/assets/IWAE_img/Code2.jpeg" width="600" height="250"></p>
<div align="center">
(코드 2)
</div> 
...
## 9. Potential Short-Coming & Future Work
...
첫 번째로 sampling방식 자체의 문제를 해결한 논문이다. 해당 논문은 IWAE의 bound를 biased estimator라고 정의하고, 그 bias를 줄이는 데 Jacknife bias correction방식을 사용한다. IWAE의 finding이 Sampling방식이기 때문에 같은 선상에서 더 나은 sampling procedure를 제시한 논문이다. 개인적으로 IWAE 논문의 가장 큰 한계점을 뽑으라면 importance sampling에서 발생하는 variance가 작다는 것을 mean absolute deviation (MAD)을 통해 low variance를 간접적으로만 증명한 것이라고 생각한다. 하지만 해당 논문은 이러한 부분을 proposition2를 통해 thorough analysis를 제공한다. 

**Proposition 2**

$$Var[\hat{L_K}] = \frac{1}{K} \frac{\mu_2}{\mu^2} - \frac{1}{K^2}(\frac{\mu_3}{\mu^3} - \frac{5\mu_{2}^{2}}{2\mu^4}) +o(K^{-2})$$
해당 proposition을 통해 K가 무한대로 갈수록 bias와 variance가 $O(1/K)$의 속도로 사라지는 것을 밝혔다. 

<p align="center"><img src="/assets/IWAE_img/Figure9.jpg" width="500" height="450"></p>
<div align="center">
...
![image](https://user-images.githubusercontent.com/84373236/205976649-8df7c79e-89f6-4515-949e-37715832f523.png)
