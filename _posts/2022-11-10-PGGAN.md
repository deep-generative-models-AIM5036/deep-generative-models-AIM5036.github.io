...
$$
\begin{align}
&s_{k} \leftarrow \beta s_{k-1} + (1-\beta)(\bigtriangledown_{\theta} L(\theta_{k}))^{2} \\
&\theta_{k+1} = \theta_{k} - \alpha {\bigtriangledown_{\theta} L(\theta_{k}) \over \sqrt{s_{k}}}
\end{align}
$$
...
$$
\begin{align}
m_{k} &= (1-\beta_{1})\bigtriangledown_{\theta} L(\theta_{k})+\beta_{1} m_{k-1} \\
v_{k} &= (1-\beta_{2})\bigtriangledown_{\theta} L(\theta_{k})^{2}+\beta_{2} v_{k-1} \\
\hat{m_{k}} &= {m_{k} \over 1-\beta_{1}^{k}} \\
\hat{v_{k}} &= {v_{k} \over 1-\beta_{2}^{k}} \\
\theta_{k+1} &= \theta_{k} - \alpha {\hat{m_{k}} \over \sqrt{\hat{v_{k}}}+\epsilon}
\end{align}
$$
...
$$
W_{i} \sim N(0, {1 \over \sqrt{{1 \over 2} D_{a}}})
$$

식에서 보면 normalization constant 는 $${1 \over \sqrt{{1 \over 2} D_{a}}}$$이다.  
