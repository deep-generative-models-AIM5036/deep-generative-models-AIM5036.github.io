...
앞서 선정된 100,000장의 라벨링된 이미지의 latent vector들을 가지고 요소 각각에 대해 linear SVM을 학습시킨 뒤, 엔트로피 $\mathrm{H}(Y|X)$를 계산하여 최종 separability score $\mathrm{exp}(\sum_{i}\mathrm{H}(Y_i \vert X_i))$를 계산하였습니다.
