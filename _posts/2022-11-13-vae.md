...
~~~
vae의 forward 함수부분입니다.
encoder 과정에서 $\mu$ 와 $\sigma$ 를 구하고 Reparametrize 를 위한 epsilon 샘플링또한 정의합니다.

latent z는 보시는바와 같이 sampled_z 처럼 training data와 샘플링된 epsilon값, 그리고 $\sigma$ 값이 모두 곱해져 $\mu$ 값과 더해져서 준비됩니다.

이러한 sampled_z는 학습 파라미터와 함께 sigmoid 함수를 거쳐 output으로 출력됩니다.

Regularization term 계산을 위한 KL loss를 위와같이 구하여 학습단계에서 최종 출력값과 loss를 전달합니다.
